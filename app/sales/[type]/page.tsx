...
// 1. CONFIGURAÇÃO DOS VÍDEOS (Vturb Smartplayer)
...
export default function SalesPage() {
  ...
  // --- CONTROLE DO DELAY (TIMER) ---
  const [showOffer, setShowOffer] = useState(false)

  useEffect(() => {
    // 7 minutos = 420 segundos = 420000 ms
    // Para testar, mude 420000 para 1000 (1 segundo)
  ...
  const handleCheckout = async (plan: 'monthly' | 'annual') => {
    ...
    // Redirect logic with check for existing '?'
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Video Section (ATUALIZADO COM VTURB) */}
          ...
          <Card className="aspect-video bg-black flex items-center justify-center overflow-hidden relative shadow-2xl rounded-xl">
             {videoEmbeds[type] ? (
               ...
               <div className="text-center text-white">
                 ...
                 <p className="text-xl">Vídeo não encontrado para: {type}</p>
                 <p className="text-sm text-gray-400">Verifique a URL ou o nome no código.</p>
               </div>
             )}
          </Card>
        </div>
      ...
      {/* --- BLOCO ESCONDIDO (APARECE APÓS 7 MINUTOS) --- */}
      {showOffer && (
        <>
          {/* 1. Pricing Section (AGORA EM PRIMEIRO) */}
          <section className="py-16 bg-gradient-to-r from-primary-500 to-secondary-500 animate-fade-in">
          ...
          {/* 2. Problem Section (AGORA EM SEGUNDO) */}
